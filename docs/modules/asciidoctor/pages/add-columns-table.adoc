...
The number of columns in a table is specified by the `cols` attribute or <<ex-number-of-columns-first-row, by the number of cells found in the first non-empty line>> after the opening table delimiter (`|===`).

[#ex-number-of-columns-cols-attribute]
== Specify the number of columns with the cols attribute

The `cols` attribute is set in the attribute list on a table block. It accepts a comma-separated list of column specifiers. Each *column specifier* represents a column and the width, alignment, and style properties assigned to that column. A column specifier is commonly represented by a number, but in some cases, can be represented by a symbol or letter. In <<ex-attribute,Example 1,>>, cols is assigned a list of four numeric column specifiers.
...
[cols="1,1,1,1"]
----
In <<ex-attribute,Example 1>> the value assigned to `cols` contains four *column specifiers*. The number of entries in the value’s list determines the number of columns in the table. That means the table in the above example will contain four columns. When the specifier is a number, such as `1` or `50`, the integer represents the width of the column in proportion to the other columns in the table. In <<ex-attribute,Example 1>>, each column will be the same width because the integer in each specifier is the same. Let’s look at the column specifiers in <<ex-cols-attribute, Example 2 >>and compare it to <<ex-attribute, Example 1.>>

.Assign column specifiers to the cols attribute
[source#ex-cols-attribute]
----
[cols="3,3,3,3"]
----

Both <<ex-attribute,Example 1>> and <<ex-cols-attribute, Example 2>> will produce tables with four columns of equal width. Let’s use the cols value in <<ex-cols-attribute, Example 2>> to create a table.

.Create a table with four columns of equal width
[source#ex-cols-equal-width]
----
[cols="3,3,3,3"] # <.>
|=== # <.>
# <.>
...
<<ex-cols-equal-width,Example 3>> creates the table displayed below.

====
include::../examples/example-table.adoc[tag=table-h]
====

As specified, the table includes four columns of equal width, a header row, and a regular row. Since all of the columns in <<ex-cols-equal-width,Example 3>> are assigned the same width via their column specifiers (i.e., 3), the number of columns could be specified with a <<column-multi,column multiplier>>. Or, you could adjust the width of an individual column by increasing the numerical value of its specifier.
...
.Represent [cols="5,5,5"] as a column multiplier.
----
[cols="3*"] # <.>
----
...
You can use a multiplier in a comma-separated list with column specifiers, too. In <<ex-assign, Example 5>>, the first column is represented by a column specifier, and the next three columns are represented by a multiplier.
...
As show below,<<ex-assign, Example 5>> creates a table containing a wide first column following by three columns of equal width.

====
include::../examples/example-table.adoc[tag=table-i]
====
...
[#ex-number-of-columns-first-row]
...
.Create a table with tree columns using its first row
[source#ex-columns-first-row]
----
|===
# <.>
...
<.>  Enter all of the first row’s cells on a single line. Each cell represents one column.
<.> The cells in subsequent rows don’t need to be entered on a single line.

The table <<ex-columns-first-row, Example 6>> has three columns since it first row contains three cells.

.Result of <<ex-columns-first-row, Example 6>>
====
include::../examples/example-table.adoc[tag=table-j]
====